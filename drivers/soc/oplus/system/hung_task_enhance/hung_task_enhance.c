...
static bool is_usersapce_key_process(struct task_struct *t)
{
	...
	if (!strcmp(t->comm, "main") && (tcred->uid.val == 0) && (t->parent != 0
			&& !strcmp(t->parent->comm, "init"))) {
		return true;
	}
	...
	if (!strncmp(t->comm, "Binder:", 7) && (t->group_leader->pid == t->pid)
			&& (tcred->uid.val == 1000) && (t->parent != 0
					&& !strcmp(t->parent->comm, "main"))) {
		return true;
	}
				  ...
				  unsigned int *iowait_count, bool *show_lock, bool *call_panic)
{
	...
	if (is_usersapce_key_process(t)) {
		if (t->state == TASK_UNINTERRUPTIBLE) {
			...
			snprintf(sysctl_hung_task_kill, HUNG_TASK_KILL_LEN, "%s,traced for %ld seconds",
				 t->comm, timeout);
		...
		printk(KERN_ERR
		       "DeathHealer: task %s:%d blocked for more than %ld seconds in state 0x%lx. Count:%d\n",
