...
static int hybridswap_info_show(struct seq_file *m, void *unused)
{
	show_val_kb(m, "SamePages:      ",
		    get_hybridswap_meminfo("same_pages"));
	show_val_kb(m, "ComprDataSize:  ",
		    get_hybridswap_meminfo("compr_data_size") >> PAGE_SHIFT);
	show_val_kb(m, "PagesStored:    ",
		    get_hybridswap_meminfo("pages_stored"));
	return 0;
}
DEFINE_PROC_SHOW_ATTRIBUTE(hybridswap_info);

static int version_show(struct seq_file *m, void *unused)
{
	seq_printf(m, "osvelte v%d.%d.%d based on kernel-5.10\n",
		   OSVELTE_MAJOR, OSVELTE_MINOR, OSVELTE_PATCH_NUM);
	return 0;
}
DEFINE_PROC_SHOW_ATTRIBUTE(version);
...
int osvelte_sys_memstat_init(void)
{
	...
	if (!gpu_root) {
		pr_err("create osvelte gpu root dir failed\n");
		return -ENOMEM;
	}
