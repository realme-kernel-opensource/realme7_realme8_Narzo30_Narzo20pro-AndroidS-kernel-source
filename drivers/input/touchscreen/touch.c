...
bool __init tp_judge_ic_match(char *tp_ic_name)
{
	...
	case 21037:
		if(strstr(tp_ic_name, "nf_nt36525b") && strstr(saved_command_line, "oplus21037_nt35625b_innolux_dphy_dsi_vdo_lcm_drv")) {
			...
			return true;
		} else if (strstr(tp_ic_name, "ili7807s") && strstr(saved_command_line, "oplus21037_ili9882n_innolux_72_dphy_dsi_vdo_lcm_drv")) {
			pr_info("tp judge ic match 21037 driver is ili7807s,IC is ili9882n,fts\n");
			return true;
		}
...
bool  tp_judge_ic_match_commandline(struct panel_info *panel_data)
{
	...
	case 21037:
		...
		if (strstr(saved_command_line, "oplus21037_ili9882n_innolux_72_dphy_dsi_vdo_lcm_drv")) {
			pr_info("[TP] touch ic = ili9882n_innolux_72_dphy\n");
			g_tp_dev_vendor = TP_INX;
			tp_used_index = ili9882n_inx_72;
		}
		break;
	case 21041:
	case 21042:
		       ...
		       struct panel_info *panel_data)
{
	...
	if (prj_id == 0x212A1 || prj_id == 21851 || prj_id == 21876) {
...
|| prj_id == 20654 || prj_id == 20108 || prj_id == 20109 || prj_id == 20307) {
	...
	if (strstr(saved_command_line, "nt36672c_boe_sala") && prj_id == 20682) {
			hw_res->TX_NUM = 18;
			hw_res->RX_NUM = 36;
		}
...
*/
	...
	case 21037:
		...
		if (tp_used_index == ili9882n_inx_72) {
			snprintf(panel_data->fw_name, MAX_FW_NAME_LENGTH,
					 "tp/21037/FW_%s_%s.img",
					 "NF_ILI9882N", "INX_72");
			if (panel_data->test_limit_name) {
			...
			panel_data->manufacture_info.fw_path = panel_data->fw_name;
			memcpy(panel_data->manufacture_info.version, "AA278IN", 7);
			panel_data->firmware_headfile.firmware_data = FW_21037_ILI9882N_INX_72;
			panel_data->firmware_headfile.firmware_size = sizeof(FW_21037_ILI9882N_INX_72);
		}
