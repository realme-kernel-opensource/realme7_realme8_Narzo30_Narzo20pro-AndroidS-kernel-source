...
#include "gc02kmipiraw_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc02k_camera_sensor"
#define LOG_1 LOG_INF("GC02K, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static kal_bool GC02KDuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02K_SENSOR_ID_20611,
	...
	.custom1 = {	//Copy from cap 24fps
		.pclk = 41000000,
		.linelength = 1079,
		.framelength = 1583,//1248
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 1600,
		.grabwindow_height = 1200,
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC02K_SENSOR_GAIN_BASE)
		reg_gain = GC02K_SENSOR_GAIN_BASE;
	else if (reg_gain > GC02K_SENSOR_GAIN_MAX)
		reg_gain = GC02K_SENSOR_GAIN_MAX;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC02K_MIRROR);
	...
	write_cmos_sensor(0x4e, GC02K_BLK_Select_H);
	write_cmos_sensor(0x4f, GC02K_BLK_Select_L);
	...
	write_cmos_sensor(0x92, GC02K_STARTY);
	write_cmos_sensor(0x94, GC02K_STARTX);
	...
	/* Shipei.Chen@Cam.Drv, 20200604, modify for streaks, default is:0x02, 0x82!*/
	write_cmos_sensor(0x02, 0x87);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == GC02K_SENSOR_ID_20630) {
				*sensor_id = imgsensor_info.sensor_id;
				pr_info("gc02k_mipi_raw_20611: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_info("gc02k_mipi_raw_20611: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			if (sensor_id == GC02K_SENSOR_ID_20630) {
				sensor_id = imgsensor_info.sensor_id;
				LOG_INF("gc02k_mipi_raw_20611: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_info("gc02k_mipi_raw_20611: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
	...
	spin_unlock(&imgsensor_drv_lock);
	GC02KDuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
