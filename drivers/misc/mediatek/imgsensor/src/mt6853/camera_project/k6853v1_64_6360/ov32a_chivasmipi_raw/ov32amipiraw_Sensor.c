...
#define PFX "ov32a1q_camera_sensor_chivas"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV32A_SENSOR_ID_CHIVAS,
...
//xtalk step2
static kal_uint16 addr_data_pair_init2_ov32a1q[] = {
	0x5500, 0x82,
	0x5501, 0x89,
	0x5502, 0x88,
	0x5503, 0x8a,
...
static kal_uint16 addr_data_pair_custom5_ov32a1q[] = {
	0x0305, 0x64,
	0x0344, 0x01,
	0x0345, 0xe0,
	0x034a, 0x05,
...
static kal_uint32 open(void) //check out
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		i++;
		if (sensor_id == imgsensor_info.sensor_id) {
			printk("ov32a_mipi_raw_chivas: i2c write id: 0x%x, sensor id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			break;
		}
		retry = 2;
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		printk("Open sensor id fail: 0x%x\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
			...
			MUINT32 framerate)
{
	...
	default:
		...
		if (imgsensor.frame_length > imgsensor.shutter)
			set_dummy();
	    printk("error scenario_id = %d,use preview scenario\n",
			scenario_id);
	break;
	}
	return ERROR_NONE;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
        *(MINT32 *)(signed long)(*(feature_data + 1)) = SENSOR_FUSION_THRESHOLD;
        break;
