...
//kal_bool GC02KDuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = ATHENSB_GC02K_SENSOR_ID,
            ...
            *sensor_id = return_sensor_id();
            if (*sensor_id == GC02K_SENSOR_ID_20630) {
                *sensor_id = imgsensor_info.sensor_id;
                LOG_INF("gc02k i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("gc02k Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
