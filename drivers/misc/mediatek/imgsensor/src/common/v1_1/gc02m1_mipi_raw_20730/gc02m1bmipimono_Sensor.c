...
#endif
#define IMGSENSOR_GC02M1_OTP_SLAVE_ADDRESS 0xA4
#define IMGSENSOR_GC02M1_OTP_MODULE_ID 0x09
...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC02M1_SENSOR_ID_20730,        //record sensor id defined in Kd_imgsensor.h
...
static void sensor_init(void)
{
	...
	table_write_cmos_sensor(addr_data_pair_init_gc02m1b,
		sizeof(addr_data_pair_init_gc02m1b) / sizeof(kal_uint16));
...
static kal_uint16 read_module_id(void)
{
    ...
    iReadRegI2C(pusendcmd, 2, (u8 *)&get_byte, 1, IMGSENSOR_GC02M1_OTP_SLAVE_ADDRESS/*EEPROM_READ_ID*/);
    printk(" gc02m10 the module id is %d\n", get_byte);
    return get_byte;
}

static kal_uint16 check_material_id(void){
    kal_uint16 get_byte = 0;
    kal_uint16 table[3] = {0x33, 0x30, 0x30};
    kal_uint16 addr = 0x00e5;
    kal_uint16 cnt = 0;
    for (;cnt < 3; cnt++){
        char pusendcmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF)};
        iReadRegI2C(pusendcmd, 2, (u8 *)&get_byte, 1, IMGSENSOR_GC02M1_OTP_SLAVE_ADDRESS/*EEPROM_READ_ID*/);
        if (get_byte != table[cnt]){
            printk("gc02m10 check material id fail! 0x%x\n",get_byte);
            return  0;
        }
        addr += 0x01;
        }
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			printk("gc02m1bMIPI_Sensor_id = 0x%x\n",*sensor_id);
            if (*sensor_id == 0x02e0) {
...
UINT32 GC02M1_MIPI_RAW_20730_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    ...
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC02M1B_MIPI_RAW_20601_SensorInit    */
