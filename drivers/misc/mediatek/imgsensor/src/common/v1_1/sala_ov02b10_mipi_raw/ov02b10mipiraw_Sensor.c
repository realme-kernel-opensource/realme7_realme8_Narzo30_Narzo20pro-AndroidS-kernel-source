...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SALA_OV02B10_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			...
			printk("OV02B10 get_imgsensor_id 0x%x module_id 0x%x\n", *sensor_id,imgsensor_info.module_id);
            if ((*sensor_id == 0x002b)
                && (imgsensor_info.module_id == IMGSENSOR_MODULE_ID_HOLITECH)) {
                *sensor_id == imgsensor_info.sensor_id;
                printk("OV02B10 get_imgsensor_id success: 0x%x\n", *sensor_id);
                #ifdef OPLUS_FEATURE_CAMERA_COMMON
                /*Degao.Lan@Camera.DRV add for register device info 20191108*/
                if (deviceInfo_register_value == 0x00) {
                    Oplusimgsensor_Registdeviceinfo("Cam_r2", sensor_name_golden_result, imgsensor_info.module_id);
    ...
    if ((*sensor_id != imgsensor_info.sensor_id)
        || imgsensor_info.module_id != IMGSENSOR_MODULE_ID_HOLITECH) {
        printk("OV02B10 get_imgsensor_id failed: 0x%x, %d\n", *sensor_id, imgsensor_info.module_id);
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id) {
        printk("OV02B10 open failed: 0x%x\n", sensor_id);
