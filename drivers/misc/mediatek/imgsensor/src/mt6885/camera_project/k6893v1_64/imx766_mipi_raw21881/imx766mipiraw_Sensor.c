...
BYTE imx766_LRC_data[352] = {0};
static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX766_SENSOR_ID_21881 - SENSOR_ID_OFFSET_21881,
...
static void read_module_data(void)
{
    ...
    gImgEepromInfo_21881.camNormdata[0][0] = Eeprom_1ByteDataRead(0x00, 0xA0);
    gImgEepromInfo_21881.camNormdata[0][1] = Eeprom_1ByteDataRead(0x01, 0xA0);
    imgsensor_info.module_id = Eeprom_1ByteDataRead(0x00, 0xA0);
    Oplusimgsensor_Registdeviceinfo(gImgEepromInfo_21881.pCamModuleInfo[0].name,
        gImgEepromInfo_21881.pCamModuleInfo[0].version, imgsensor_info.module_id);
    for(i = 2; i < 8; i++) {
        gImgEepromInfo_21881.camNormdata[0][i] = Eeprom_1ByteDataRead(0x04+i, 0xA0);
    }

    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
        gImgEepromInfo_21881.camNormdata[0][8+i] = Eeprom_1ByteDataRead(0xB0+i, 0xA0);
    }
    ...
    memcpy(&gImgEepromInfo_21881.camNormdata[2][28], &Dac_master, 4);
    memcpy(&gImgEepromInfo_21881.camNormdata[0][40], &vcmID, 4);
    memcpy(&gImgEepromInfo_21881.camNormdata[0][44], &lensID, 4);
    memcpy(&gImgEepromInfo_21881.camNormdata[0][48], &Dac_mac, 4);
    memcpy(&gImgEepromInfo_21881.camNormdata[0][52], &Dac_inf, 4);
    /*Read stereo eeprom data*/
    for (i = 0; i < CALI_DATA_MASTER_LENGTH; i ++) {
        gImgEepromInfo_21881.stereoMWdata[i] = Eeprom_1ByteDataRead(IMX766_STEREO_START_ADDR+i, 0xA0);
    }
    gImgEepromInfo_21881.i4CurSensorIdx = 0;
    gImgEepromInfo_21881.i4CurSensorId = imgsensor_info.sensor_id;
}
...
static kal_uint32 seamless_switch(enum MSDK_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
    ...
    case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
    {
        ...
        FMC_GPH_START;
        imx766_table_write_cmos_sensor(imx766_seamless_preview, sizeof(imx766_seamless_preview) / sizeof(kal_uint16));
        if (ae_ctrl) {
    ...
    case MSDK_SCENARIO_ID_CUSTOM7:
    {
        ...
        FMC_GPH_START;
        imx766_table_write_cmos_sensor(imx766_seamless_custom7, sizeof(imx766_seamless_custom7) / sizeof(kal_uint16));
        if (ae_ctrl) {
    ...
    case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
    {
        ...
        FMC_GPH_START;
        imx766_table_write_cmos_sensor(imx766_seamless_normal_video, sizeof(imx766_seamless_normal_video) / sizeof(kal_uint16));
        if (ae_ctrl) {
    ...
    case MSDK_SCENARIO_ID_CUSTOM4:
    {
        ...
        FMC_GPH_START;
        imx766_table_write_cmos_sensor(imx766_seamless_custom4, sizeof(imx766_seamless_custom4) / sizeof(kal_uint16));
        if (ae_ctrl) {
