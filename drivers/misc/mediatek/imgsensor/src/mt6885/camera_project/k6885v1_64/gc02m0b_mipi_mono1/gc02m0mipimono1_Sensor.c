...
#define PFX "GC02M0_camera_sensor_mono1"
...
static imgsensor_info_struct imgsensor_info = {
       ...
       .checksum_value = 0x5e4a9c4c,    //checksum value for Camera Auto Test
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("gc02mo_mipi_mono1 I2C_BUS = %d\n", I2C_BUS);
	if(I2C_BUS != 4){
		*sensor_id = 0xFFFFFFFF;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			if (deviceInfo_register_value == 0x00) {
				/*
                                register_imgsensor_deviceinfo("Cam_r3", DEVICE_VERSION_GC02M0,
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id() + 1;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("i2c mono1 write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("Read mono1 sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
