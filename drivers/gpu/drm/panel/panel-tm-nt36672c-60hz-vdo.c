...
static void lcm_panel_init(struct lcm *ctx)
{
    ...
    lcm_dcs_write_seq_static(ctx ,0xFF, 0x23);/* 12bit PWM */
	lcm_dcs_write_seq_static(ctx ,0xFB, 0x01);/* 12bit PWM */
	lcm_dcs_write_seq_static(ctx ,0x00, 0x80);/* 12bit PWM */
    ...
    lcm_dcs_write_seq_static(ctx, 0x3B, 0x14,0x36,0x10,0x04,0x04);
				 ...
				 unsigned int level)
{
	...
	printk("[lcm] lcm_setbacklight_cmdq level = %d  \n",level);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (ret < 0)
		return ret;
#endif
	register_device_proc("lcd","nt36672c_tm","TM");
	pr_info("%s-\n", __func__);
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-tm-nt36672c-60hz-vdo",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("tm nt36672c VDO 60HZ LCD Panel Driver");
