...
static int lcm_panel_ldo3_enable(struct device *dev)
{
	...
	if (ret < 0)
		DISP_ERR("set voltage disp_ldo3 fail, ret = %d\n", ret);
	retval |= ret;

	/* enable regulator */
	ret = regulator_enable(disp_ldo3);
	if (ret < 0)
		DISP_ERR("enable regulator disp_ldo3 fail, ret = %d\n", ret);
	retval |= ret;
	DISP_INFO("get lcm_panel_ldo3_enable\n");
...
static int lcm_panel_ldo3_disable(struct device *dev)
{
	...
	if (ret < 0)
		DISP_ERR("disable regulator disp_ldo3 fail, ret = %d\n", ret);
	retval |= ret;
	DISP_INFO("disable regulator disp_ldo3\n");
...
static unsigned long panel_doze_get_mode_flags(struct drm_panel *panel, int doze_en)
{
	...
	DISP_INFO("mode flags =%d, doze_en = %d\n", mode_flags, doze_en);
	return mode_flags;
}
...
static int panel_doze_enable_start(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	...
	//cmd = 0x10;
	//cb(dsi, handle, &cmd, 1);
	usleep_range(16*1000, 16*1000+100);
	DISP_INFO("Successful\n");
...
static int lcm_panel_disp_off(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	cmd = 0x10;
	cb(dsi, handle, &cmd, 1);
	usleep_range(120*1000, 120*1000+100);
	DISP_INFO("Successful\n");
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	DISP_INFO("[soso]21127 samsung probe\n");
	if (dsi_node) {
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "oplus21127_samsung_ams643ag01_1080p_dsi_cmd,lcm",
		.owner = THIS_MODULE,
