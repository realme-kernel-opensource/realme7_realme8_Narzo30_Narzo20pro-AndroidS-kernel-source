...
struct LCM_setting_table lcm_seed_setting_20638[] = {
	{REGFLAG_CMD,3,{0xF0, 0x5A, 0x5A}},
	{REGFLAG_CMD,2,{0x80, 0x92}},
	{REGFLAG_CMD,2,{0xB1, 0x00}},
	{REGFLAG_CMD,3,{0xB0, 0x2B,0xB1}},
...
struct LCM_setting_table lcm_setbrightness_normal_20638[] = {
...
struct LCM_setting_table lcm_setbrightness_hbm_20638[] = {
	{REGFLAG_CMD,3, {0x51,0x00,0x00}},
	{REGFLAG_CMD,3, {0xF0,0x5A,0x5A}},
...
struct LCM_setting_table lcm_finger_HBM_on_setting_20638[] = {
	{REGFLAG_CMD,3, {0xF0,0x5A,0x5A}},
...
struct LCM_setting_table lcm_normal_HBM_on_setting_20638[] = {
	{REGFLAG_CMD,3, {0xF0,0x5A,0x5A}},
...
static int panel_doze_enable_start(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	...
	/* Zhijun.Ye@MM.DisplayDriver.AOD, 2021/09/28, add for aod */
	...
	/* #ifdef OPLUS_BUG_STABILITY */
	/* Chenjun@MM.DisplayDriver.AOD, 2022/02/21, add for aod timming*/
	msleep(DELAY_80MS);
	pr_err("%s sleep over\n", __func__);
	/* #endif */
...
static int oplus_lcm_dc_backlight(void *dsi, dcs_write_gce cb,void *handle,unsigned int level,int hbm_en)
{
	...
	seed_table = kmemdup(lcm_seed_setting_20638, sizeof(lcm_seed_setting_20638), GFP_KERNEL);
	if (!seed_table)
		goto dc_disable;

	for (i = 0; i < sizeof(lcm_seed_setting_20638)/sizeof(lcm_seed_setting_20638[0]); i++) {
...
dc_disable:
	if (oplus_dc_alpha) {
		for (i = 0; i < sizeof(lcm_seed_setting_20638)/sizeof(lcm_seed_setting_20638[0]); i++){
			cb(dsi, handle, lcm_seed_setting_20638[i].para_list, lcm_seed_setting_20638[i].count);
		}
		pr_err("exit DC");
	}
			      ...
			      dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	if (level > BRIGHTNESS_HALF) {
		...
		lcm_setbrightness_hbm_20638[0].para_list[1] = BL_LSB;
		lcm_setbrightness_hbm_20638[0].para_list[2] = BL_MSB;

		for (i = 0; i < sizeof(lcm_setbrightness_hbm_20638)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_setbrightness_hbm_20638[i].para_list, lcm_setbrightness_hbm_20638[i].count);
		}
	} else {
		BL_LSB = level >> 8;
		BL_MSB = level & 0xFF;

		lcm_setbrightness_normal_20638[0].para_list[1] = BL_LSB;
		lcm_setbrightness_normal_20638[0].para_list[2] = BL_MSB;

		for (i = 0; i < sizeof(lcm_setbrightness_normal_20638)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_setbrightness_normal_20638[i].para_list, lcm_setbrightness_normal_20638[i].count);
		}
	}
}
		...
		void *handle, unsigned int hbm_mode)
{
	...
	if(hbm_mode == 1) {
	//	oppo_lcm_dc_backlight(dsi,cb,handle, oplus_display_brightness, 1);
		for (i = 0; i < sizeof(lcm_finger_HBM_on_setting_20638)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_finger_HBM_on_setting_20638[i].para_list, lcm_finger_HBM_on_setting_20638[i].count);
		}
	} else if (hbm_mode == 0) {
		/*level = oppo_lcm_dc_backlight(dsi,cb,handle, oplus_display_brightness, 0);*/
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "oppo20638,s68fc01,lcm", },
	{ }
};
