...
uint8_t aw_pa_mode_v2 = AW_CTOS_PROFILE_MUSIC;
int aw87xxx_spk_low_voltage_status_v2 = 0;
#endif /* CONFIG_SND_SOC_OPLUS_PA_MANAGER */

/*****************************************************************
* aw87xxx marco
******************************************************************/
#define AW87XXX_I2C_NAME	"aw87xxx_pa_v2"
...
int aw87xxx_update_profile(struct aw87xxx *aw87xxx, char *profile)
{
	...
	aw87xxx_monitor_stop_v2(&aw87xxx->monitor);
	if (0 == strncmp(profile, aw87xxx->prof_off_name, AW_PROFILE_STR_MAX)) {
		ret = aw87xxx_power_down(aw87xxx, profile);
	} else {
		ret = aw87xxx_power_on(aw87xxx, profile);
		if (!ret)
			aw87xxx_monitor_start_v2(&aw87xxx->monitor);
	}
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
void aw87xxx_pa_audio_spk_low_voltage_status(int bStatus)
{
        struct list_head *pos = NULL;
        struct aw87xxx *aw87xxx = NULL;
        int ret = 0;

        aw87xxx_spk_low_voltage_status_v2 = bStatus;
        ...
        list_for_each(pos, &g_aw87xxx_list) {
                ...
                if (aw87xxx->aw_dev.hwen_status == 1) {
                        if (aw87xxx_spk_low_voltage_status_v2 > 0) {
                                pr_info("%s: aw87xxx write vol reg 0x04 when playing\n", __func__);
                                ret = aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev, 0x04, 0x08);
                                if(ret < 0)
                                        AW_LOGE("aw87xxx set [0x04] = 0x08 failed\n");
                        } else {
                                pr_info("%s: aw87xxx write vol reg normal when playing\n", __func__);
                                ret = aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev, 0x04, 0x18);
                                if(ret < 0)
                                        AW_LOGE("aw87xxx set [0x04] = 0x18 failed\n");
                        }
                }
        }
}

void aw87xxx_enable_pa_v2(int enable, int mode, int32_t channel) {
        if(enable) {
                if(mode == SPK_MODE){
                        aw87xxx_set_profile_by_id(channel, AW_CTOS_PROFILE_MUSIC);
                }else{
                        aw87xxx_set_profile_by_id(channel, AW_CTOS_PROFILE_RECEIVER);
                }
        } else {
...
void aw87xxx_enable_l_pa_v2(int enable, int mode){
        aw87xxx_enable_pa_v2(enable, mode, AW87XXX_LEFT_CHANNEL);
}

void aw87xxx_enable_r_pa_v2(int enable, int mode){
        aw87xxx_enable_pa_v2(enable, mode, AW87XXX_RIGHT_CHANNEL);
}

int ext_amp_low_voltage_set_v2(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
        if (ucontrol->value.integer.value[0] >= 2) {
                aw87xxx_pa_audio_spk_low_voltage_status(0);
        } else {
                aw87xxx_pa_audio_spk_low_voltage_status(1);
        }
...
int ext_amp_low_voltage_get_v2(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.integer.value[0] = aw87xxx_spk_low_voltage_status_v2;
        pr_info("%s: aw87xxx_spk_low_voltage_status_v2 = %d\n", __func__, aw87xxx_spk_low_voltage_status_v2);
        return 0;
}

int aw87xxx_spk_mode_set_v2(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
        ...
        if(ucontrol->value.integer.value[0] == 0){
                aw_pa_mode_v2 = AW_CTOS_PROFILE_MUSIC;
        }else if(ucontrol->value.integer.value[0] == 1){
                aw_pa_mode_v2 = AW_CTOS_PROFILE_VOICE;
        }else if(ucontrol->value.integer.value[0] == 2){
                aw_pa_mode_v2 = AW_CTOS_PROFILE_FM;
        }

        return 0;
}

int aw87xxx_spk_mode_get_v2(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
        pr_info("%s() = %d\n", __func__, aw_pa_mode_v2);
        //kcontrol enum 0~2 is "Music", "Voice", "Fm"
        //aw87xxx_scene_mode enum 1~3 is "Music", "Voice", "Fm", so need subtract 1
        if(aw_pa_mode_v2 == AW_CTOS_PROFILE_FM){
                ucontrol->value.integer.value[0] = 2;
                return 0;
        }
        ucontrol->value.integer.value[0] = aw_pa_mode_v2 - 1;
        return 0;
}

bool is_mute_status_v2 = false;
int aw87xxx_speaker_mute_set_v2(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
        struct soc_enum *e = (struct soc_enum *)kcontrol->private_value;
        ...
        if(ucontrol->value.integer.value[0]){
                aw87xxx_set_profile_by_id(AW87XXX_RIGHT_CHANNEL, AW_CTOS_PROFILE_OFF);
                aw87xxx_set_profile_by_id(AW87XXX_LEFT_CHANNEL, AW_CTOS_PROFILE_OFF);
                is_mute_status_v2 = true;
        }else{
                aw87xxx_set_profile_by_id(AW87XXX_RIGHT_CHANNEL, aw_pa_mode_v2);
                aw87xxx_set_profile_by_id(AW87XXX_LEFT_CHANNEL, aw_pa_mode_v2);
                is_mute_status_v2 = false;
        }

        return 0;
}

int aw87xxx_speaker_mute_get_v2(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
        pr_info("%s() = %d\n", __func__, is_mute_status_v2);
        ucontrol->value.integer.value[0] = is_mute_status_v2;
        return 0;
}
#endif /*CONFIG_SND_SOC_OPLUS_PA_MANAGER*/
				...
				const struct i2c_device_id *id)
{
	...
	/*monitor init*/
	aw87xxx_monitor_init_v2(aw87xxx->dev, &aw87xxx->monitor, dev_node);
	...
	mutex_unlock(&g_aw87xxx_mutex_lock);
        ...
        if ((speaker_device !=NULL) && (aw87xxx->dev_index == 0)) {
                ...
                speaker_device->speaker_enable_set = aw87xxx_enable_l_pa_v2;
                speaker_device->boost_voltage_set = ext_amp_low_voltage_set_v2;
                speaker_device->boost_voltage_get = ext_amp_low_voltage_get_v2;
                speaker_device->spk_mode_set = aw87xxx_spk_mode_set_v2;
                speaker_device->spk_mode_get = aw87xxx_spk_mode_get_v2;
                speaker_device->speaker_mute_set = aw87xxx_speaker_mute_set_v2;
                speaker_device->speaker_mute_get = aw87xxx_speaker_mute_get_v2;
        } else if ((speaker_device !=NULL) && (aw87xxx->dev_index == 1)) {
                speaker_device->chipset = MFR_AWINIC;
                speaker_device->type = R_SPK;
                speaker_device->vdd_need = 0;
                speaker_device->speaker_enable_set = aw87xxx_enable_r_pa_v2;
                speaker_device->boost_voltage_set = ext_amp_low_voltage_set_v2;
                speaker_device->boost_voltage_get = ext_amp_low_voltage_get_v2;
                speaker_device->spk_mode_set = aw87xxx_spk_mode_set_v2;
                speaker_device->spk_mode_get = aw87xxx_spk_mode_get_v2;
        ...
        if (is_new_speaker_device) {
                oplus_pa_aw87xxx_node_v2 = oplus_speaker_pa_register(speaker_device);
        }
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
        oplus_speaker_pa_remove(oplus_pa_aw87xxx_node_v2);
#endif /*CONFIG_SND_SOC_OPLUS_PA_MANAGER*/
