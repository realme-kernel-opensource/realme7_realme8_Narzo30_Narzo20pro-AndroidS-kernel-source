...
static int aw882xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (aw882xx->index == 0) {
#else /* OPLUS_ARCH_EXTENDS */
		if (aw882xx->index == 0 && aw882xx->spin_flag) {
#endif /* OPLUS_ARCH_EXTENDS */
...
static int aw882xx_dynamic_create_controls(struct aw882xx *aw882xx)
{
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%u_prof", aw882xx->index);
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%u_switch", aw882xx->index);
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%u_monitor", aw882xx->index);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (aw_dev_get_profile_name(aw882xx->aw_pa, name, aw_dev_get_profile_index(aw882xx->aw_pa)) == 0) {
...
static void aw882xx_add_codec_mute_controls(struct aw882xx *aw882xx)
{
	if (aw882xx->aw_pa->channel == 0x00) {
		snd_soc_add_codec_controls(aw882xx->codec,
...
{
	char buf[50];
	unsigned int i2cbus = aw882xx->i2c->adapter->nr;
	unsigned short i2caddr = aw882xx->i2c->addr;
				...
				int num_dai)
{
	...
	int i;
	unsigned int i2cbus = aw882xx->i2c->adapter->nr;
	unsigned short addr = aw882xx->i2c->addr;
				...
				loff_t *ppos)
{
	...
	ret = oplus_aw_cali_attr_show_re(&i2c->dev, str);
	pr_warning("%s addr 0x%x, str=%s\n", __func__, aw882xx->i2c->addr, str);
	ret = simple_read_from_buffer(user_buf, count, ppos, str, ret);
	kfree(str);
re_show_err:
...
extern ssize_t oplus_aw_cali_attr_re_show(struct device *dev, char *buf);
				...
				loff_t *ppos)
{
	...
	ret = oplus_aw_cali_attr_re_show(&i2c->dev, str);
	pr_warning("%s addr 0x%x, str=%s\n", __func__, aw882xx->i2c->addr, str);
	ret = simple_read_from_buffer(user_buf, count, ppos, str, ret);
	kfree(str);
cali_re_err:
...
const struct file_operations aw882xx_dbgfs_cali_re_fops = {
	.open = simple_open,
	.read = aw882xx_dbgfs_check_cali_re,
	.llseek = default_llseek,
};
